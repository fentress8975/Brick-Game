...
public class GameNetworkHandler : SingletonNetWork<GameNetworkHandler>
{
    ...
    private void CalculatePlayersRTT()
    {
        SendPlayerPingClientRpc(NetworkManager.Singleton.NetworkConfig.NetworkTransport.GetCurrentRtt(m_Player1Id),Player1RpcParams);
        SendPlayerPingClientRpc(NetworkManager.Singleton.NetworkConfig.NetworkTransport.GetCurrentRtt(m_Player2Id), Player2RpcParams);
    }
    ...
    [ClientRpc]
    private void SendPlayerPingClientRpc(ulong RTT, ClientRpcParams player2RpcParams)
    {
        PlayerPing = (int)RTT;
    }
}
